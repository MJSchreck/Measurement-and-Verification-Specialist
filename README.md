# Measurement-and-Verification-Specialist

GSA Energy Division M&V Specialist knowledge base and workflow documentation for ESPC/UESC portfolio management.

## Overview

This repository contains documentation and tools for managing Zone 7 M&V operations across 16 ESPC/UESC contracts ($286M+ portfolio, 76 facilities) in Regions 8, 9, and 10.

## Contents

- **GSA-Energy-COR-Initialization.md** - Core initialization script defining role, portfolio context, workflows, and technical reference materials
- **folder-structure-reference.md** - Google Drive folder structure, filing rules, and contract-specific locations
- **portfolio-dashboard.html** - Interactive portfolio command center dashboard (open in browser)
- **zone7-master-tracker.md** - Consolidated tracking file with contracts, M&V status, payments, and critical issues
- **portfolio-context.md** - Quick reference for Claude AI context
- **email-triage-handoff.md** - Email processing guide with categorization rules and progress tracker (2000 emails)
- **cor-onboarding-checklist.md** - COR takeover duties, CO meeting schedules, and designation tracking

## Portfolio Coverage (14 Active Contracts)

| Contractor | Contracts | CO |
|------------|-----------|-----|
| McKinstry | DFC (47PJ0024F0020) | Felipe Jolles |
| Ameresco | San Diego NDER2, Harold Washington, WY-MT, Dakotas | Felipe/Jerrud |
| PG&E | UESC Sansome (GS-P-09-17-KS-0009) | Felipe Jolles |
| SDG&E | UESC San Diego (47PK0222F0014) | Felipe Jolles |
| ABM | LA Phase I & IIA (47PK0324C0001) | Heidi Johnson |
| Honeywell | LA NDER2, SF NDER2, Detroit ENABLE | Heidi/Jerrud |
| Noresco | Chicago NDER1 (47PF0023F0723) | Krystal Blue |
| Trane | Battle Creek NDER1 (47PF0024F0107) | Miles Conant |
| JCI | PJKK Hawaii (47PK0223F0041) | Miles Conant |

## Key Workflows

1. EASi Receiving Report Processing
2. M&V Report Review
3. Government Witnessing Protocol
4. CPARS Evaluation Process

## Recent Updates

- **2026-02-09**: Added cor-onboarding-checklist.md with COR takeover duties and CO meeting schedule
- **2026-02-09**: Received COR Designation Letter for UESC Sansome (GSP0917KS0009) from Felipe Jolles
- **2026-02-04**: Added email-triage-handoff.md for processing 2000 emails with categorization rules and batch tracking
- **2026-02-04**: Added zone7-master-tracker.md with consolidated contract, M&V, payment, and issue tracking
- **2026-02-03**: SF ESPC Honeywell folder restructure (Patrick Chapman) - New COR tabs and shared M&V working folder created
