# Harold Washington Social Security Center (ENABLE)

## Identifiers
- **Contract/PIID:** GSP0517GB0001
- **ESCO:** Ameresco Inc
- **Type:** ENABLE
- **Region:** GSA R5
- **Contracting Officer:** Jerrud Parker
- **COR EASi (current):** Johnny Zuzic
- **Proposed COR:** Matthew Schreck (Nomination Letter Issued: X)
- **Award Date:** 10/28/2016
- **End Date / Ultimate Completion:** 10/31/2027
- **Obligated:** $17,193,924.68
- **Total Contract Value:** $17,727,105.68
- **Payment Date:** November

## Status (per 6/9/2026 COR tracker)
- EMP2 reviewed Year 7 and prepared the Instructional Memo (IM).
- Matt to confirm he's taking this one solo for Year 8 going forward.

## ECMs
- (to be extracted from shared drive)

## Performance Year Savings History
| PY | Report Period | Proposed | Verified/Calculated | Shortfall | Status |
|----|---------------|----------|----------------------|-----------|--------|
|    |               |          |                      |           |        |

## Open Issues / Flags
- COR transition from Johnny Zuzic to Matthew Schreck pending confirmation for Year 8 onward.

## Key Reviewers/Contacts
- Jerrud Parker – Contracting Officer
- Johnny Zuzic – current COR of record

## FY26–FY28 Funding Forecast (Annual Funding Requests workbook)
Note: appears in the funding workbook as **"CHI SSA ESPC"** (Chicago Social Security Administration), PIID GS-P-05-17-GB-0001 — matches GSP0517GB0001 above.

| FY | Perf Period | Amount Required | Payment Date | Date Mod Needed |
|----|-------------|------------------|--------------|------------------|
| FY26 | PY9 | $153,908.00 | 2026-11-01 | 2025-10-01 |

**Latest status note (FY26 tracker):** Need to confirm the "date mod needed" and when funding should be requested — an RWA (Reimbursable Work Authorization) should be requested from SSA by 5/1/2026.

## Drive Folder Structure (Shared Drive)
Found under the national ESPC/UESC portfolio drive (`0ALoTdmHUP-XoUk9PVA`) → NDER 1 → `R5 Chicago SSA - Ameresco`. Uses per-modification numbered subfolders under a "34" tab folder: `21 - PO21 - PY8 Pmt` and `22 - PS22 - PY7 Rpt` — confirming **PY7 report and PY8 payment are the current live items**, consistent with the "Matt to confirm he's taking this for Year 8" note above.

## Not Yet Reviewed
- Contents of the `21 - PO21 - PY8 Pmt` and `22 - PS22 - PY7 Rpt` folders — structure confirmed but documents not yet opened. This drive appears sparser than others (only 2 items found at this level) — may need to check for additional Tab folders not yet surfaced.
